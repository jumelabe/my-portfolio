...
const Portfolio: React.FC = () => {
  const projects = [
    ...
    },
    {
      ...
      description:
        "A management system for community homeowner associations designed to streamline operations.",
      ...
      <div className="w-full max-w-6xl mx-auto text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, index) => (
            <div
              key={index}
              className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-xl hover:-translate-y-2 transition-all duration-300 flex flex-col group h-full"
            >
              ...
              <div className="p-6 flex flex-col flex-grow text-left">
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-xs font-semibold text-primary bg-blue-50 px-2 py-1 rounded"
                    >
                      {tag}
                    </span>
                  ))}
